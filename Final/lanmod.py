...
original_vocab_size = 2722
pre = "pre."
preprocess_vocab_size = 1798
string_to_write = "";
...
vocab_size = preprocess_vocab_size
...
file_name = results_dir + pre + "IS=" + str(init_scale) + "_LR=" + str(learning_rate) + "_MGN=" + str(max_grad_norm) + "_NL=" + str(num_layers) + "_NS=" + str(num_steps) + "_HS=" + str(hidden_size) + "_ME=" + str(max_epoch) + "_MME=" + str(max_max_epoch) + "_KP=" + str(keep_prob) + "_D=" + str(decay) + ".txt"
...
def run_epoch(session, m, data, eval_op, verbose=False):
    ...
    for step, (x, y) in enumerate(reader.ptb_iterator(data, m.batch_size, m.num_steps)):
        ...
        if verbose and step % (epoch_size // 10) == 10:
            ...
            message = "%.3f perplexity: %.3f speed: %.0f wps" % (step * 1.0 / epoch_size, np.exp(costs / iters),
...
with tf.Graph().as_default(), tf.Session() as session:
    ...
    if os.path.isfile(file_name):
        print("You have already done this setting")
        exit()
    ...
    for i in range(max_max_epoch):
        ...
        # Place holder
        message = "Epoch %d : Learning rate: %.3f" % (i + 1, session.run(m.lr))
        string_to_write += message + "\n"
        print(message)
        # Run the loop for this epoch in the training model
        train_perplexity = run_epoch(session, m, train_data, m.train_op,
                                   verbose=True)
        message = "Epoch %d : Train Perplexity: %.3f" % (i + 1, train_perplexity)
        string_to_write += message + "\n"
        print("Epoch %d : Train Perplexity: %.3f" % (i + 1, train_perplexity))
        
        # Run the loop for this epoch in the validation model
        valid_perplexity = run_epoch(session, mvalid, valid_data, tf.no_op())
        message = "Epoch %d : Valid Perplexity: %.3f" % (i + 1, valid_perplexity)
        string_to_write += message + "\n"
        print(message)
    
        words = get_word(session, mtest, "the", 19, tf.no_op(), word_to_id, verbose=False)
        message = 'Sample sentence: ' + ' '.join(words)
        string_to_write += message + "\n"
        print(message)

    # Run the loop in the testing model to see how effective was our training
    test_perplexity = run_epoch(session, mtest, test_data, tf.no_op())
    message = "Test Perplexity: %.3f" % test_perplexity
    string_to_write += message
    print(message)
