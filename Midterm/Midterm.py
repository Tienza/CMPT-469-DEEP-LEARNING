...
class TextLoader():
    def __init__(self, data_dir, batch_size, seq_length, encoding='utf-8'):
        ...
        tensor_file = os.path.join(data_dir, "data.npyy")
...
seq_length = sequence # RNN sequence length
...
learning_rate = 0.002
decay_rate = 0.97
rnn_size = hidden_states # size of RNN hidden state (output dimension)
...
class LSTMModel():
    def __init__(self, sample=False):
        rnn_size = hidden_states  # size of RNN hidden state vector
        ...
        outputs, last_state = tf.contrib.legacy_seq2seq.rnn_decoder(inputs, self.initial_state, self.stacked_cell,
                                                                    loop_function=None, scope='rnnlm_class1')
        output = tf.reshape(tf.concat(outputs, 1), [-1, rnn_size])
