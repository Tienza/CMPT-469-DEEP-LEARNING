...
def next_batch_train(num, data, labels):
    global trainImages
    global trainLabels
...
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters:
        ...
        batch_x, batch_y = next_batch_train(batch_size, trainImages, trainLabels)
    ...
    for i in range(len(test_label)):
        ...
        tarY = np.asarray(tarY, dtype=np.float32)
        predY = np.asarray(predY, dtype=np.float32)
        res.append(np.fabs(tarY - predY))
        step += 1

    res = np.asarray(res, dtype=np.float32)
    testing_average = np.mean(res)
    print("**************************************************************")
    print("Testing Mean: " + str(testing_average))
